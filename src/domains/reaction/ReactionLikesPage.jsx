...
export default function ReactionLikesPage() {
  ...
  // 🔥 userid 또는 page 바뀔 때만 호출 (StrictMode 중복 방지)
  useEffect(() => {
    if (userid) loadData();
  }, [page, userid]);
  ...
  const loadData = async () => {
    ...
    try {
      const res = await axiosInstance.get(
        `/reactions/user/${userid}/type/like`,
        { params: { page, size } }
      );
  ...
  const cancelLike = async (reactionNo) => {
    if (!window.confirm("좋아요를 취소할까요?")) return;
  ...
  return (
    <div className="reaction-container">
      <h2 className="reaction-title">❤️ 내가 좋아요 한 게시글</h2>
          ...
          {filtered.map((r) => (
            <div key={r.reactionNo} className="reaction-card">
              ...
              <button
                ...
                onClick={() => cancelLike(r.reactionNo)}
              >
                좋아요 취소
              </button>
            </div>
