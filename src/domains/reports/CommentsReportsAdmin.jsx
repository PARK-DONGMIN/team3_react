...
export default function CommentsReportsAdmin() {
  ...
  const load = async () => {
    try {
      const res = await axiosInstance.get("/reports/comments/search", {
  ...
  const updateStatus = async (reportId, status) => {
    try {
      await axiosInstance.put(
        `/reports/comments/${reportId}/status?status=${status}&adminId=${adminId}`
      );
      alert("처리 완료");
      load();
    } catch {
  ...
  const approve = (reportId) => {
    if (!window.confirm("승인을 하면 댓글이 삭제됩니다.\n진행할까요?")) return;
  ...
  const deleteReport = async (reportId) => {
    ...
    try {
      await axiosInstance.delete(`/reports/comments/${reportId}`);
      alert("삭제 완료");
      load();
    } catch {
      alert("삭제 실패");
  ...
  return (
    <div className="admin-container">
      <h2 className="admin-title">💬 댓글 신고 관리</h2>
