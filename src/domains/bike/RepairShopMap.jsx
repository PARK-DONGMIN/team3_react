...
const RepairShopMap = () => {
  ...
  const clearMarkers = () => {
    ...
    openedInfoWindowRef.current.close();   // 🔥 이 줄이 핵심
  }
    ...
    data.forEach((item) => {
      ...
      const content = `
        ...
        ">
          ...
          🕒 운영시간: ${item.openTime ?? "정보 없음"}<br/>
          🚚 출장 서비스: ${item.onsiteService ? "가능" : "불가능"}
        </div>
      `;
  ...
  const fetchItemsByBounds = async (bounds) => {
    ...
    try {
      const res = await axiosInstance.get("/api/bike/repairs/within", {
        params: {
          swLat: sw.getLat(),
          swLng: sw.getLng(),
          neLat: ne.getLat(),
